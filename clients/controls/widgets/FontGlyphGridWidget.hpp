...
signals:
    void sizeSelected(uint16_t size);
...
private:
    ...
    QRect headerContentRect(const QRect& header) const;
    QRect disclosureRect(const QRect& header) const;
    ...
    uint16_t                        m_selectedSizeOnly  = 0U;
};
